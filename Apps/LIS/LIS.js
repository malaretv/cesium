...
function init() {
  ...
  function cartesianToDummyPolar(cartesianCoords) {
    ...
    var polarCartesianCoords = Cesium.Cartesian3.clone(cartesianCoords);
  ...
  function dummyPolarToCartesian(cartesianCoords) {
    // rotate along y axis by -90 deg
    // multiple vector my rotation matrix
    // R1 = (0, 0, -1)
    //     (0, 1, 0)
  ...
  function adjustCartesianCoords(cartesianCoords) {
  ...
  function invAdjustCartesianCoords(cartesianCoords) {
    if (isOptimizedPolarTerrain) {
      // adjust for dummy polar case
  ...
  function updateBodiesPosToDummyPolar() {
    ...
    sunLightDirectionSPICE = adjustCartesianCoords(sunLightDirectionSPICE);
    ...
    sunPosSPICE = adjustCartesianCoords(sunPosSPICE);
    // EARTH position
    ...
    earthLightDirectionSPICE = adjustCartesianCoords(earthLightDirectionSPICE);
    ...
    earthPosSPICE = adjustCartesianCoords(earthPosSPICE);
  }
  ...
  function updateBodiesPos() {
    ...
    if (
      ...
      earthPosSPICE.x =
        (bodiesPos.EARTH.position[0] * 1000.0) / earthScaleFactor;
      earthPosSPICE.y =
        (bodiesPos.EARTH.position[1] * 1000.0) / earthScaleFactor;
      earthPosSPICE.z =
        (bodiesPos.EARTH.position[2] * 1000.0) / earthScaleFactor;
      earthPosSPICE = adjustCartesianCoords(earthPosSPICE);
  ...
  function newTerrainNameSelected(terrainName) {
    if (
      terrainName === "automatic" ||
      terrainName === "automatic - no normals"
    ) {
  ...
  function maybeUpdateTerrainProvider(lat, height) {
    ...
    if (!lat || !height) {
      ...
      ellipsoid.cartesianToCartographic(
        invAdjustCartesianCoords(camera.positionWC),
        cartographicCamera
      );
  ...
  viewer.scene.canvas.addEventListener("mousemove", function (e) {
    ...
    if (cartesian) {
      cartographic = ellipsoid.cartesianToCartographic(
        invAdjustCartesianCoords(cartesian)
      );
  ...
  function saveStateToQueryString() {
    ...
    var camera_direction = Cesium.Cartesian3.pack(
      invAdjustCartesianCoords(camera.direction),
      []
    );
    var camera_up = Cesium.Cartesian3.pack(
      invAdjustCartesianCoords(camera.up),
      []
    );
