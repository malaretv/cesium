...
(function () {
  ...
  const pickerDateTimeChangedEvent = new Event("pickerDateTimeChanged");
  const pickerClosedEvent = new Event("pickerClosed");
  ...
  DTS.prototype.setup = function () {
    ...
    this.inputElem.addEventListener("click", handler, false);
  ...
  DTBox.prototype.setup = function () {
    ...
    this.addHandler("visible", function (state, prevState) {
      if (state.visible && !prevState.visible) {
        ...
        self.elem.dispatchEvent(pickerClosedEvent);
      }
    });
  };
    ...
    if ("DAYS" == this.bodyType) {
      ...
      if (!toAppend) {
        toAppend = makeGrid(
          ...
          "cal-months",
          null,
          this.onMonthSelected.bind(this)
        );
        for (var i = 0; i < 3; i++) {
          for (var j = 0; j < 4; j++) {
  ...
  DTBox.prototype.setBodyContent = function () {
    ...
    if ("DAYS" == this.bodyType) {
      ...
      var adjusted = new Date(
        start.getTime() - oneDayMilliSecs * start.getDay()
      );
  ...
  DTBox.prototype.setInputValue = function () {
    ...
    this.elem.dispatchEvent(pickerDateTimeChangedEvent);
  };
  ...
  DTBox.prototype.onDateSelected = function (e) {
    ...
    if (!(row.nextSibling && row.nextSibling.nextSibling) && date < 8) {
      ...
      !(row.previousSibling && row.previousSibling.previousSibling) &&
      date > 7
    ) {
      this.month -= 1;
    }
